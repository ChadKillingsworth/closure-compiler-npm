...
module.exports = function(initOptions) {
  ...
  return function (compilationOptions, pluginOptions) {
    ...
    var loadMaps = true;
    if ('loadMaps' in pluginOptions) {
      loadMaps = pluginOptions.loadMaps;
    }
    ...
    if (pluginOptions.includeDependencies) {
      combinedPipe = combinedPipe
          ...
          .pipe(function() {
            var deps = moduleDeps({ fileCache: fileCache});

            // Closure-compiler needs the package.json files as well
            deps.on('package', function(pkg) {
              var packagePath = pkg.__dirname + '/package.json';
              if (!(packagePath in packages)) {
                packages[packagePath] = {
                  file: packagePath,
                  source: JSON.stringify(pkg, null, 0)
                };
              }
            });
          ...
          .pipe(function() { return new DepsToVinyl(packages); })
          .pipe(function() { return sourcemaps.init({loadMaps: loadMaps})});
    } else if (pluginOptions.loadMaps) {
      combinedPipe = combinedPipe
          .pipe(function() { return sourcemaps.init({loadMaps: loadMaps})});
    }
    combinedPipe = combinedPipe
