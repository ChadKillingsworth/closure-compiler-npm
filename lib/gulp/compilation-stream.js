...
function CompilationStream(compilationOptions, pluginOptions) {
  ...
  if (!Array.isArray(compilationOptions)) {
    this.compilatonOptions_ = {};

    // convert camel-case options to underscores
    for (var opt in compilationOptions) {
      if (compilationOptions.hasOwnProperty(opt)) {
        var key = opt.replace(/[A-Z]/g, function(match) {
          return '_' + match.toLowerCase();
        });
        this.compilatonOptions_[key] = compilationOptions[opt];
      }
    }
  } else {
  ...
  this.logger_ = pluginOptions.logger || logger;
