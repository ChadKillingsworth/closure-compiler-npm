...
const isPackageVersionPublished = async (packageName, version) =>
  fetch(`https://registry.npmjs.org/${encodeURI(packageName)}/${version}`)
      .then((res) => res.ok);

const isValidPackagePath = async (packageDir) => {
...
const getPackageInfo = async (packageDir) => ({
  path: packageDir,
  pkg: JSON.parse(await fs.readFile(`${packageDir}/package.json`, 'utf8'))
});

const publishPackagesIfNeeded = async (packageInfo) => {
